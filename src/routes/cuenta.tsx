...
function CuentaPage() {
  ...
  async function deleteData() {
    if (
      !auth.user ||
      !confirm(
        "¿Eliminar tus horarios, asistencia, promedio y progreso guardados? Esta acción no se puede deshacer.",
      )
    )
      return;
    await execute(async () => {
  ...
  async function deleteAccount() {
    if (
      !confirm(
        "¿Eliminar definitivamente tu cuenta y todos sus datos? Esta acción no se puede deshacer.",
      )
    )
      return;
    await execute(async () => {
      ...
      <main className="mx-auto max-w-4xl px-6 pb-24 pt-12 sm:pt-20">
        <Reveal>
          ...
          <p className="mt-3 max-w-2xl text-muted-foreground">
            Guardá tu horario, asistencia, promedio y progreso para recuperarlos desde cualquier
            dispositivo.
          </p>
        </Reveal>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm leading-relaxed text-muted-foreground">
                    El sistema de cuentas ya está preparado, pero faltan{" "}
                    <code>VITE_SUPABASE_URL</code> y <code>VITE_SUPABASE_ANON_KEY</code>. Hasta
                    configurarlas, tus herramientas siguen funcionando normalmente con
            ...
            <div className="mx-auto mt-10 max-w-md rounded-2xl border border-border bg-card p-6 shadow-xl">
              <div className="mb-6 grid grid-cols-2 rounded-xl bg-muted/50 p-1">
                <button
                  ...
                  className={`rounded-lg px-3 py-2 text-sm font-medium transition ${mode === "login" ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}
                >
                  Iniciar sesión
                </button>
                <button
                  ...
                  className={`rounded-lg px-3 py-2 text-sm font-medium transition ${mode === "signup" ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}
                >
                  Crear cuenta
                </button>
              </div>
              <div className="mb-5 rounded-xl border border-primary/20 bg-primary/5 p-3 text-xs leading-relaxed text-muted-foreground">
                {mode === "login" ? (
                  ...
                  <ol className="space-y-1">
                    <li>
                      <strong className="text-foreground">1.</strong> Ingresá un correo al que
                      tengas acceso.
                    </li>
                    <li>
                      <strong className="text-foreground">2.</strong> Creá y repetí una contraseña
              ...
              <form onSubmit={submit} className="space-y-4">
                ...
                {mode === "signup" && (
                  <>
                    ...
                    {confirmPassword && (
                      <p
                        className={`text-xs ${password === confirmPassword ? "text-emerald-500" : "text-amber-500"}`}
                      >
                ...
                {message && (
                  <p className="rounded-xl bg-emerald-500/10 px-3 py-2 text-xs text-emerald-400">
                    {message}
                  </p>
                )}
                <button
                  disabled={busy}
                  className="btn-premium flex w-full items-center justify-center gap-2 rounded-xl bg-primary px-4 py-3 text-sm font-semibold text-primary-foreground disabled:opacity-50"
                >
                  {busy ? (
            ...
            <Reveal>
              <div className="h-full rounded-2xl border border-border bg-card p-6">
                ...
                <div className={`mt-5 flex items-center gap-2 text-sm ${syncMeta.color}`}>
                  <syncMeta.Icon
                    className={`h-4 w-4 ${auth.syncStatus === "syncing" ? "animate-spin" : ""}`}
                  />
                  {syncMeta.label}
                </div>
                {auth.syncError && <p className="mt-2 text-xs text-red-400">{auth.syncError}</p>}
                <button
                  onClick={() => execute(auth.syncNow)}
                  disabled={busy || auth.syncStatus === "syncing"}
            ...
            <Reveal>
              <div className="h-full rounded-2xl border border-border bg-card p-6">
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Se sincronizan horario, asistencia, promedio, calculadora de notas, énfasis y
                  progreso de mallas.
                </p>
                <div className="mt-5 flex flex-wrap gap-2">
                  ...
                  <button
                    onClick={deleteData}
                    className="inline-flex items-center gap-2 rounded-xl border border-red-500/30 px-4 py-2 text-sm text-red-400 hover:bg-red-500/10"
                  >
            ...
            <Reveal className="md:col-span-2">
              <div className="rounded-2xl border border-border bg-card p-6">
                <h2 className="flex items-center gap-2 font-semibold text-foreground">
                  <KeyRound className="h-4 w-4 text-primary" /> Seguridad de la cuenta
                </h2>
                {error && (
